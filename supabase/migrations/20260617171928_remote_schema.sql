...
CREATE OR REPLACE FUNCTION "public"."set_business_hours"("p_business_id" "uuid", "p_start" time without time zone, "p_end" time without time zone, "p_tz" "text" DEFAULT 'America/Montevideo'::"text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  if not exists (
    ...
    -- and m.role in ('owner','admin')   -- descomentá para limitar a admins
  ) then
    raise exception 'no sos miembro de este negocio';
  end if;
  update businesses
     set answer_start = p_start, answer_end = p_end, answer_tz = p_tz
   where id = p_business_id;
end; $$;


ALTER FUNCTION "public"."set_business_hours"("p_business_id" "uuid", "p_start" time without time zone, "p_end" time without time zone, "p_tz" "text") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."businesses" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "answer_start" time without time zone DEFAULT '00:00:00'::time without time zone NOT NULL,
    "answer_end" time without time zone DEFAULT '23:59:00'::time without time zone NOT NULL,
    "answer_tz" "text" DEFAULT 'America/Montevideo'::"text" NOT NULL
);
...
REVOKE ALL ON FUNCTION "public"."set_business_hours"("p_business_id" "uuid", "p_start" time without time zone, "p_end" time without time zone, "p_tz" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."set_business_hours"("p_business_id" "uuid", "p_start" time without time zone, "p_end" time without time zone, "p_tz" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."set_business_hours"("p_business_id" "uuid", "p_start" time without time zone, "p_end" time without time zone, "p_tz" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."set_business_hours"("p_business_id" "uuid", "p_start" time without time zone, "p_end" time without time zone, "p_tz" "text") TO "service_role";
